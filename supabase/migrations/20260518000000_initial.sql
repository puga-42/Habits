...
comment on column public.profiles.avatar_original_url is
  'Original uncropped avatar photo URL for re-editing.';
comment on column public.profiles.avatar_crop_params is
  'Crop rectangle {x, y, width, height} applied to produce avatar_url.';
...
comment on function public.get_user_profile_page(uuid, uuid) is
  'Profile data, friendship status, and mutual friend count for a user profile page.';
comment on function public.get_user_visible_habits(uuid, uuid) is
  'Habits visible to the viewer on a target user''s profile.';
comment on function public.get_user_feed_page(uuid, uuid, timestamptz, uuid, int) is
  'Paginated feed scoped to a single user, with optional lineage filter.';
comment on function public.get_mutual_friends(uuid, uuid, int) is
  'Profiles of users who are friends with both p_user_a and p_user_b.';
comment on function public.get_user_activity_heatmap(uuid, uuid, date, date, uuid) is
  'Daily completion counts for a user over a date range, for the activity heatmap.';
...
grant execute on function public.get_user_activity_heatmap(uuid, uuid, date, date, uuid)
  to authenticated;
