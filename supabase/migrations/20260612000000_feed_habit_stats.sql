-- Feed habit stats: surface each habit's description, all-time completion count
-- (lineage-wide), and the raw material a client needs to compute the current
-- streak. Streak itself is computed client-side (RRULE expansion is client-only
-- in this codebase) — this RPC just ships the completion/skip history.
--
-- Reverses the prior "no streaks" stance (see CONTEXT.md § Stats and history,
-- revised 2026-06-12). Extends fetch_feed_page only; no schema changes.
...
as $$
  ...
  select page.id,
         ...
         -- scheduled, period_start for flex), newest first. Capped: a streak
         -- longer than ~100 occurrences is under-reported, never over-reported.
         coalesce((
         ...
         -- Last 100 skip-override dates across the lineage, newest first. Skips
         -- are neutral in a streak (neither count nor break).
         coalesce((
